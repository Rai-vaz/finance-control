        ...
        <tbody>
            {
                dataList.map((obj) => (
                        <tr key={Math.random()}>
                            ...
                            <td>{obj.flow}<span>{obj.flow === 'Entrada' && <FaRegArrowAltCircleUp className='svg-entrada'/> }{obj.flow === 'Saída' && <FaRegArrowAltCircleDown className='svg-saída'/>}{obj.flow != undefined && <FaTrashAlt className='delete-icon' onClick={deleteItem}/>}</span></td>
                        </tr>
                    )
                )
            }
